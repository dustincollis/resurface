-- Company rollup count correction.
--
-- The first rollup RPC used capped list CTEs for both display rows and
-- counts, which made counts max out at 10. Keep display lists capped, but
-- count against the full matching commitment / idea sets.
...
begin
  ...
  company_commitments_top as (
    select *
    from company_commitments_all
    order by do_by asc nulls last, created_at desc
    limit 10
  ),
  company_ideas_all as (
    select
      i.id,
      i.title,
      i.status,
  ...
  company_ideas_top as (
    select *
    from company_ideas_all
    order by created_at desc
    limit 10
  )
  select
    (select count(*)::int from company_people) as people_count,
    (select count(*)::int from company_commitments_all) as open_commitments_count,
    (select count(*)::int from company_ideas_all) as open_ideas_count,
    coalesce(
      (
        select jsonb_agg(
          jsonb_build_object(
    ...
    coalesce(
      (
        ...
        from company_commitments_top cc
      ),
      '[]'::jsonb
    ) as open_commitments,
    coalesce(
